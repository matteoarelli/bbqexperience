...
- 🚧 **v1.1 Content Depth & Growth Loop** — Phases 10–17 (planned 2026-04-15, Phase 17 added 2026-05-25)
...
- [x] 14-02-PLAN.md — Collection listing + detail pages (EN/IT/ES) + CollectionBadge on recipe detail
...
**Goal**: Search Console diventa la fonte primaria di verità per content generation, refresh e SEO meta — la pipeline smette di essere cieca su query reali e CTR effettivo. Lift atteso: 10–30× CTR sui ~50 articoli BBQ già live entro 30 giorni (baseline 2026-05-25: 35 clicks / 29k impressions / CTR 0.12% / position 6.7), +30–50% search-intent fit sui nuovi articoli, refresh loop self-sustaining.
**Depends on**: Phase 15 (umami_client.py + traffic_score), Phase 16 (Claude review gate pattern riutilizzato per meta-changes). GSC service account già funzionante (C7 IG bot, 2026-05-25).
**Requirements**: SEO-08, SEO-09, SEO-10, SEO-11, SEO-12 (da derivare in plan-phase)
**Success Criteria** (what must be TRUE):
  1. `scripts/agents/lib/gsc_client.py` esiste come port del client di `~/instagram-bot/gsc_client.py` su .119, condivide la stessa SA key (`~/secrets/gsc-merchant-sync.json`), espone `top_queries`, `top_pages`, `search_analytics(dimensions=...)`, `request_indexing(url)`, `inspect_url(site_url, url)`, ha retry/timeout coerenti col resto dello stack, e i test unitari coprono parsing + error paths.
  2. `scripts/agents/meta_optimizer.py` gira ogni giorno su Hetzner (es. 04:30 UTC dopo umami_feedback), seleziona articoli pubblicati con impressions ≥100/28gg AND CTR < benchmark per posizione (3% @ pos 6-10, 5% @ pos 1-5), passa al drafter Qwen le top 3-5 query GSC reali della pagina con prompt CTR-optimized (numero, power-words, anno solo se evergreen, max 60 char seo_title / 155 char seo_description), poi sottopone il proposal allo stesso Claude quality gate degli articoli (auto-apply su pass, `needs_human` su reject); ogni cambio finisce in `state/meta_changes.jsonl` con before/after/query_targets/decision.
  3. `scripts/agents/keyword_scout.py` integra una nuova fonte "striking-distance" che pulla GSC (`dimensions=["query"]`, range 28gg, filter pos∈[8,20] AND impressions≥30) e fonde i candidati con quelli di Google Suggest prima del dedup/quality-filter; il report Telegram weekly distingue le due fonti e indica per ogni candidato la metrica GSC d'origine.
  4. `scripts/agents/content_generator.py` (sia path single-shot che `generate_article_multistep`) riceve, quando il `target_keyword` ha dati GSC, le top 5 query reali del cluster (clicks + impressions + ctr + position) e le inietta nell'outline prompt e nel FAQ generator; in assenza di dati GSC il comportamento attuale resta invariato (graceful degrade).
  5. Dopo ogni `publish_article` su Strapi (incluso quality-gate promotion da `draft_review` a `published`), la pipeline chiama `gsc_client.request_indexing(url)` per il locale appena pubblicato e per le traduzioni IT/ES quando vengono promosse; failure dell'Indexing API logga warning ma non blocca il publish.
  6. `scripts/agents/gsc_refresh.py` gira ogni settimana (es. domenica 08:00 dopo claude_strategist), seleziona top 10 pagine BBQ con decadenza ≥30% (clicks 7gg vs media 28gg) OR impressions ≥500/28gg AND CTR <1.5%, recupera contenuto Strapi + top GSC query + un competitor diff (riusa competitor_monitor state se disponibile), passa a Claude Opus per una versione migliorata (intro riscritta sulle query reali, FAQ aggiornata, sezioni espanse), promuove dietro lo stesso quality gate e re-indicizza via Indexing API.
  7. `scripts/agents/claude_strategist.py` riceve, in aggiunta a Umami/Strapi/queue, una sezione "GSC weekly digest" con delta clicks/impressions/CTR/position vs 7gg precedenti, top 10 striking-distance, top 10 CTR-opportunity (pos 1-10 con CTR < benchmark), top 10 declining; il report settimanale Telegram include almeno 3 raccomandazioni ancorate a queste metriche.
  8. Schema markup audit: ogni blog-post/tutorial/recipe/review serve JSON-LD `FAQPage` strutturato quando il contenuto ha una sezione FAQ (parsing automatico in `web/src/lib/structured-data.ts` o equivalente), i tutorial servono `HowTo` schema con `step` array, e l'`Article` schema include `speakable` selector per il primo paragrafo + headings principali; `web/scripts/sweep_pages.py` verifica zero regressioni.
  9. Tutti i nuovi cron sono registrati su Hetzner `~/.crontab` con commenti di provenienza, ogni agente nuovo ha un report Telegram (success summary + actionable next step su empty/error), e i test unitari coprono almeno: GSC client parsing, meta_optimizer benchmark logic, keyword_scout fusion sources, content_generator GSC priming graceful-degrade, gsc_refresh selection criteria.
**Plans**: 4 plans
Plans:
- [x] 17-01-PLAN.md — Foundation: lib/gsc_client.py + lib/indexnow_client.py + lib/atomic_io.py + pytest scaffold + content_generator publish-hook indexing
- [x] 17-02-PLAN.md — meta_optimizer (Hetzner Qwen daily 03:30 UTC) + meta_review (Windows Claude gate daily 09:00) + keyword_scout GSC striking-distance + content_generator GSC priming + X-Skip-Rebuild webhook exclusion
- [x] 17-03-PLAN.md — gsc_refresh (Hetzner Sun 08:00 UTC) + gsc_refresh_review (Windows Claude Opus Sun 10:00) + claude_strategist GSC weekly digest
- [x] 17-04-PLAN.md — Schema markup audit (FAQPage/HowTo/speakable) + sweep_pages.py --check schema + per-agent Telegram coverage + cron_registry.md canonical
...
| 17. GSC-Driven Content Pipeline | v1.1 | 4/4 | Complete | 2026-05-26 |
